...
[e,function(b,a){if(200<a.statusCode&&300>a.statusCode){var e=convertedItem.pop();if(204==a.statusCode){if(a.headers.ETag){var d=e.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===$data.ConcurrencyMode.Fixed});d&&d[0]&&(e[d[0].name]=a.headers.ETag)}}else e.getType().memberDefinitions.getPublicMappedProperties().forEach(function(c){c.computed&&(e[c.name]=c.concurrencyMode===$data.ConcurrencyMode.Fixed?a.headers.ETag:b[c.name])},this);c.success&&
c.success(convertedItem.length)}else c.error(a)},c.error];this.providerConfiguration.user&&(b[0].user=this.providerConfiguration.user,b[0].password=this.providerConfiguration.password||"");this.context.prepareRequest.call(this,b);OData.request.apply(this,b)},_saveBatch:function(b,a,c){batchRequests=[];convertedItem=[];for(a=0;a<b.length;a++)for(var e=0;e<b[a].length;e++){convertedItem.push(b[a][e].data);var d={};d.headers={"Content-Id":convertedItem.length};switch(b[a][e].data.entityState){case $data.EntityState.Unchanged:continue;
case $data.EntityState.Added:d.method="POST";d.requestUri=b[a][e].entitySet.name;d.data=this.save_getInitData(b[a][e],convertedItem);break;case $data.EntityState.Modified:d.method="MERGE";d.requestUri=b[a][e].entitySet.name;d.requestUri+="("+this.getEntityKeysValue(b[a][e])+")";this.save_addConcurrencyHeader(b[a][e],d.headers);d.data=this.save_getInitData(b[a][e],convertedItem);break;case $data.EntityState.Deleted:d.method="DELETE";d.requestUri=b[a][e].entitySet.name;d.requestUri+="("+this.getEntityKeysValue(b[a][e])+
")";this.save_addConcurrencyHeader(b[a][e],d.headers);break;default:Guard.raise(new Exception("Not supported Entity state"))}batchRequests.push(d)}b=[{requestUri:this.providerConfiguration.oDataServiceHost+"/$batch",method:"POST",data:{__batchRequests:[{__changeRequests:batchRequests}]}},function(a,b){if(202==b.statusCode){for(var e=a.__batchResponses[0].__changeResponses,d=0;d<e.length;d++){var j=convertedItem[d];if(204==e[d].statusCode){if(e[d].headers.ETag){var k=j.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===
$data.ConcurrencyMode.Fixed});k&&k[0]&&(j[k[0].name]=e[d].headers.ETag)}}else j.getType().memberDefinitions.getPublicMappedProperties().forEach(function(a){a.computed&&(j[a.name]=a.concurrencyMode===$data.ConcurrencyMode.Fixed?e[d].headers.ETag:e[d].data[a.name])},this)}c.success&&c.success(convertedItem.length)}else c.error(b)},c.error,OData.batchHandler];this.providerConfiguration.user&&(b[0].user=this.providerConfiguration.user,b[0].password=this.providerConfiguration.password||"");this.context.prepareRequest.call(this,
b);OData.request.apply(this,b)},save_getInitData:function(b,a){b.physicalData=this.context._storageModel.getStorageModel(b.data.getType()).PhysicalType.convertTo(b.data,a);var c={};b.physicalData.getType().memberDefinitions.asArray().forEach(function(a){if(a.kind==$data.MemberTypes.navProperty||a.kind==$data.MemberTypes.complexProperty||a.kind==$data.MemberTypes.property&&!a.notMapped)c[a.name]=b.physicalData[a.name]},this);return c},save_addConcurrencyHeader:function(b,a){var c=b.data.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===
