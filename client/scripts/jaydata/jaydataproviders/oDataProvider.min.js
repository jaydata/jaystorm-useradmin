...
"string"===typeof c?[{cnt:c}]:c,a.success(b))},function(b,c,e){a.error(e||new Exception("Request failed","RequestError",arguments))}];this.providerConfiguration.user&&(c[0].user=this.providerConfiguration.user,c[0].password=this.providerConfiguration.password||"");this.context.prepareRequest.call(this,c);OData.request.apply(this,c)},_compile:function(b){return(new $data.storageProviders.oData.oDataCompiler).compile(b)},saveChanges:function(b,a){0<a.length?this.saveInternal(this.buildIndependentBlocks(a),
0,b):b.success(0)},saveInternal:function(b,a,c){1<b.length||1==b.length&&1<b[0].length?this._saveBatch(b,a,c):this._saveRest(b,a,c)},_saveRest:function(b,a,c){batchRequests=[];convertedItem=[];for(var e,a=0;a<b.length;a++)for(var d=0;d<b[a].length;d++)switch(convertedItem.push(b[a][d].data),e={requestUri:this.providerConfiguration.oDataServiceHost+"/",headers:{}},b[a][d].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:e.method="POST";e.requestUri+=b[a][d].entitySet.name;
e.data=this.save_getInitData(b[a][d],convertedItem);break;case $data.EntityState.Modified:e.method="MERGE";e.requestUri+=b[a][d].entitySet.name;e.requestUri+="("+this.getEntityKeysValue(b[a][d])+")";this.save_addConcurrencyHeader(b[a][d],e.headers);e.data=this.save_getInitData(b[a][d],convertedItem);break;case $data.EntityState.Deleted:e.method="DELETE";e.requestUri+=b[a][d].entitySet.name;e.requestUri+="("+this.getEntityKeysValue(b[a][d])+")";this.save_addConcurrencyHeader(b[a][d],e.headers);break;
default:Guard.raise(new Exception("Not supported Entity state"))}b=[e,function(b,a){if(200<a.statusCode&&300>a.statusCode){var e=convertedItem[0];if(204==a.statusCode){if(a.headers.ETag){var d=e.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===$data.ConcurrencyMode.Fixed});d&&d[0]&&(e[d[0].name]=a.headers.ETag)}}else e.getType().memberDefinitions.getPublicMappedProperties().forEach(function(c){c.computed&&(e[c.name]=c.concurrencyMode===$data.ConcurrencyMode.Fixed?
a.headers.ETag:b[c.name])},this);c.success&&c.success(convertedItem.length)}else c.error(a)},c.error];this.providerConfiguration.user&&(b[0].user=this.providerConfiguration.user,b[0].password=this.providerConfiguration.password||"");this.context.prepareRequest.call(this,b);OData.request.apply(this,b)},_saveBatch:function(b,a,c){batchRequests=[];convertedItem=[];for(a=0;a<b.length;a++)for(var e=0;e<b[a].length;e++){convertedItem.push(b[a][e].data);var d={};d.headers={"Content-Id":convertedItem.length};
switch(b[a][e].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:d.method="POST";d.requestUri=b[a][e].entitySet.name;d.data=this.save_getInitData(b[a][e],convertedItem);break;case $data.EntityState.Modified:d.method="MERGE";d.requestUri=b[a][e].entitySet.name;d.requestUri+="("+this.getEntityKeysValue(b[a][e])+")";this.save_addConcurrencyHeader(b[a][e],d.headers);d.data=this.save_getInitData(b[a][e],convertedItem);break;case $data.EntityState.Deleted:d.method=
"DELETE";d.requestUri=b[a][e].entitySet.name;d.requestUri+="("+this.getEntityKeysValue(b[a][e])+")";this.save_addConcurrencyHeader(b[a][e],d.headers);break;default:Guard.raise(new Exception("Not supported Entity state"))}batchRequests.push(d)}b=[{requestUri:this.providerConfiguration.oDataServiceHost+"/$batch",method:"POST",data:{__batchRequests:[{__changeRequests:batchRequests}]}},function(a,b){if(202==b.statusCode){for(var e=a.__batchResponses[0].__changeResponses,d=0;d<e.length;d++){var j=convertedItem[d];
if(204==e[d].statusCode){if(e[d].headers.ETag){var k=j.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===$data.ConcurrencyMode.Fixed});k&&k[0]&&(j[k[0].name]=e[d].headers.ETag)}}else j.getType().memberDefinitions.getPublicMappedProperties().forEach(function(a){j[a.name]=a.concurrencyMode===$data.ConcurrencyMode.Fixed?e[d].headers.ETag:e[d].data[a.name]},this),j.changedProperties=void 0}c.success&&c.success(convertedItem.length)}else c.error(b)},
c.error,OData.batchHandler];this.providerConfiguration.user&&(b[0].user=this.providerConfiguration.user,b[0].password=this.providerConfiguration.password||"");this.context.prepareRequest.call(this,b);OData.request.apply(this,b)},save_getInitData:function(b,a){b.physicalData=this.context._storageModel.getStorageModel(b.data.getType()).PhysicalType.convertTo(b.data,a);var c={};b.physicalData.getType().memberDefinitions.asArray().forEach(function(a){if(a.kind==$data.MemberTypes.navProperty||a.kind==
$data.MemberTypes.complexProperty||a.kind==$data.MemberTypes.property&&!a.notMapped)c[a.name]=b.physicalData[a.name]},this);return c},save_addConcurrencyHeader:function(b,a){var c=b.data.getType().memberDefinitions.getPublicMappedProperties().filter(function(a){return a.concurrencyMode===$data.ConcurrencyMode.Fixed});c&&c[0]&&(a["If-Match"]=b.data[c[0].name],b.data[c[0].name]="")},getTraceString:function(b){this._compile(b);return b},supportedDataTypes:{value:[$data.Integer,$data.String,$data.Number,
$data.Blob,$data.Boolean,$data.Date,$data.Object],writable:!1},supportedBinaryOperators:{value:{equal:{mapTo:"eq",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression,$data.Expressions.OrderExpression]},notEqual:{mapTo:"ne",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression,$data.Expressions.OrderExpression]},equalTyped:{mapTo:"eq",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression,$data.Expressions.OrderExpression]},notEqualTyped:{mapTo:"ne",dataType:"boolean",
...
parameters:[{name:"filter",dataType:"$data.Queryable"}],mapTo:"any",frameType:$data.Expressions.SomeExpression},every:{invokable:!1,allowedIn:[$data.Expressions.FilterExpression],parameters:[{name:"filter",dataType:"$data.Queryable"}],mapTo:"all",frameType:$data.Expressions.EveryExpression},take:{},skip:{},orderBy:{},orderByDescending:{},first:{},include:{},batchDelete:{}},enumerable:!0,writable:!0},fieldConverter:{value:{fromDb:{"$data.Integer":function(b){return"string"===typeof b&&/^\d+$/.test(b)?
parseInt(b):b},"$data.Number":function(b){return b},"$data.Date":function(b){return b?new Date(parseInt(b.substr(6))):void 0},"$data.String":function(b){return b},"$data.Boolean":function(b){return b},"$data.Blob":function(b){return b},"$data.Object":function(b){return void 0===b?new $data.Object:"string"===typeof b?JSON.parse(b):b},"$data.Array":function(b){return void 0===b?new $data.Array:b instanceof $data.Array?b:JSON.parse(b)}},toDb:{"$data.Entity":function(b){return"'"+JSON.stringify(b.initData)+
"'"},"$data.Integer":function(b){return b},"$data.Number":function(b){return 0==b%1?b:b+"m"},"$data.Date":function(b){return b?"datetime'"+b.toISOString()+"'":null},"$data.String":function(b){return"'"+b.replace(/'/g,"''")+"'"},"$data.Boolean":function(b){return b?"true":"false"},"$data.Blob":function(b){return b},"$data.Object":function(b){return JSON.stringify(b)},"$data.Array":function(b){return JSON.stringify(b)}}}},getEntityKeysValue:function(b){for(var a=[],c=void 0,e=b.entitySet.createNew.memberDefinitions.asArray(),
d=0,g=e.length;d<g;d++){var f=e[d];if(f.key){c=b.data[f.name];switch(f.dataType){case "Edm.Guid":c="guid'"+c+"'";break;case "Edm.Binary":c="binary'"+c+"'";break;case "Edm.Byte":c="0123456789ABCDEF"[d>>4&15]+"0123456789ABCDEF"[d&15];break;case "Edm.DateTime":c="datetime'"+c.toISOString()+"'";break;case "Edm.Decimal":c+="M";break;case "Edm.Single":c+="f";break;case "Edm.Int64":c+="L";break;case "Edm.String":case "string":c="'"+c+"'"}a.push(f.name+"="+c)}}return 1<a.length?a.join(","):c}},null);
