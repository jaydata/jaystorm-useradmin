...
b.headers.Etag;else{var f=g.fieldConverter.fromDb[Container.resolveType(c.type)];d[c.name]=f?f(a[c.name]):a[c.name]}},this);c.success&&c.success(f.length)}else c.error(b)},c.error];this.appendBasicAuth(a[0],this.providerConfiguration.user,this.providerConfiguration.password,this.providerConfiguration.withCredentials);this.context.prepareRequest.call(this,a);OData.request.apply(this,a)},_saveBatch:function(a,b,c){for(var b=[],f=[],d=0;d<a.length;d++)for(var e=0;e<a[d].length;e++){f.push(a[d][e].data);
var g={};g.headers={"Content-Id":f.length};switch(a[d][e].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:g.method="POST";g.requestUri=a[d][e].entitySet.name;g.data=this.save_getInitData(a[d][e],f);break;case $data.EntityState.Modified:g.method="MERGE";g.requestUri=a[d][e].entitySet.name;g.requestUri+="("+this.getEntityKeysValue(a[d][e])+")";this.save_addConcurrencyHeader(a[d][e],g.headers);g.data=this.save_getInitData(a[d][e],f);break;case $data.EntityState.Deleted:g.method=
"DELETE";g.requestUri=a[d][e].entitySet.name;g.requestUri+="("+this.getEntityKeysValue(a[d][e])+")";this.save_addConcurrencyHeader(a[d][e],g.headers);break;default:Guard.raise(new Exception("Not supported Entity state"))}b.push(g)}var h=this,a=[{requestUri:this.providerConfiguration.oDataServiceHost+"/$batch",method:"POST",data:{__batchRequests:[{__changeRequests:b}]}},function(a,b){if(202==b.statusCode){for(var d=a.__batchResponses[0].__changeResponses,g=[],e=0;e<d.length;e++)if(200<d[e].statusCode&&
...
else{var b=h.fieldConverter.fromDb[Container.resolveType(a.type)];j[a.name]=b?b(d[e].data[a.name]):d[e].data[a.name]}},this)}else g.push(d[e]);0<g.length?c.error(g):c.success&&c.success(f.length)}else c.error(b)},c.error,OData.batchHandler];this.appendBasicAuth(a[0],this.providerConfiguration.user,this.providerConfiguration.password,this.providerConfiguration.withCredentials);this.context.prepareRequest.call(this,a);OData.request.apply(this,a)},save_getInitData:function(a,b){a.physicalData=this.context._storageModel.getStorageModel(a.data.getType()).PhysicalType.convertTo(a.data,
...
mapTo:"any",frameType:$data.Expressions.SomeExpression},every:{invokable:!1,allowedIn:[$data.Expressions.FilterExpression],parameters:[{name:"filter",dataType:"$data.Queryable"}],mapTo:"all",frameType:$data.Expressions.EveryExpression},take:{},skip:{},orderBy:{},orderByDescending:{},first:{},include:{},batchDelete:{}},enumerable:!0,writable:!0},fieldConverter:{value:{fromDb:{"$data.Integer":function(a){return"string"===typeof a&&/^\d+$/.test(a)?parseInt(a):a},"$data.Number":function(a){return a},
