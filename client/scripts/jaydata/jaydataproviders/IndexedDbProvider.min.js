...
onabort:b.error,oncomplete:function(){c.operationProvider?(c.operationProvider.storageProvider.providerConfiguration.source[d.tableName]=a.rawDataList,c.operationProvider.storageProvider.executeQuery(a,{success:function(a){a.expression.nodeType===$data.Expressions.ExpressionType.Count&&(a.rawDataList[0]={cnt:a.rawDataList[0]});b.success(a)},error:b.error})):b.success(a)}}).objectStore(d.tableName);Container.createModelBinderConfigCompiler(a,[]).Visit(a.expression);if(c.operationProvider)e.openCursor().onsuccess=
function(b){if(b=b.target.result)a.rawDataList.push(b.value),b["continue"]()};else switch(a.expression.nodeType){case $data.Expressions.ExpressionType.Count:e.count().onsuccess=function(b){a.rawDataList.push({cnt:b.target.result})};break;default:e.openCursor().onsuccess=function(b){if(b=b.target.result)a.rawDataList.push(b.value),b["continue"]()}}},_getKeySettings:function(a){var b=this,c={autoIncrement:!1},d=[];a.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(a){a.key&&
d.push(a.name);a.computed&&(a.key||Guard.raise(new Exception("Only key field can be a computed field!")),a=Container.resolveName(a.type),!0===b.supportedAutoincrementKeys[a]&&(c.autoIncrement=!0))});1<d.length?(c.autoIncrement&&Guard.raise(new Exception("Auto increment is only valid for a single key!")),c.keys=d):1==d.length?c.keyPath=d[0]:Guard.raise(new Exception("No valid key found!"));return c},saveChanges:function(a,b){function c(){if(0===e.length)a.success();else{var b={},h=e.shift().map(function(a){b[a.entitySet.tableName]=
