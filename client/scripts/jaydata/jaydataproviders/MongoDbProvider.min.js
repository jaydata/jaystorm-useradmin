...
this._getServer(),{})).open(function(c,d){if(c)b.error(c);else{var f=function(c,d){var e=0,f;for(f in a.context._entitySetReferences)a.context._entitySetReferences.hasOwnProperty(f)&&e++;for(f in a.context._entitySetReferences)a.context._entitySetReferences.hasOwnProperty(f)&&d.dropCollection(a.context._entitySetReferences[f].tableName,function(){0==--e&&(b.success(a.context),d.close())})};a.providerConfiguration.username?d.authenticate(a.providerConfiguration.username,a.providerConfiguration.password||
"",function(a,c){a?b.error(a):c&&f(a,d)}):f(c,d)}});break;default:b.success(this.context)}},executeQuery:function(b,a){var c=this,a=$data.typeSystem.createCallbackSetting(a),d=b.context.getEntitySetFromElementType(b.defaultType);(new $data.storageProviders.mongoDB.mongoDBCompiler).compile(b);(new this.driver.Db(this.providerConfiguration.databaseName,this._getServer(),{})).open(function(f,h){if(f)a.error(f);else{var g=new c.driver.Collection(h,d.tableName),e=b.find,n=function(d,e){d?a.error(d):(b.rawDataList=
e instanceof Array?e:[{cnt:e}],b.context=c.context,a.success(b),h.close())},s=function(){switch(b.expression.nodeType){case $data.Expressions.ExpressionType.Count:g.find(e.query,e.options).count(n);break;case $data.Expressions.ExpressionType.BatchDelete:g.remove(e.query,{safe:!0},n);break;default:g.find(e.query,e.options).toArray(n)}};c.providerConfiguration.username?h.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(b,c){b?a.error(b):c&&s()}):s()}})},_typeFactory:function(b,
a,c){b=Container.resolveName(b);return c&&c[b]?c[b](a):new (Container.resolveType(b))(a)},_saveCollections:function(b,a){var c=this,d=0,f=this._getServer(),h=0,g=function(a){0==--h&&a()},e=function(a,e,f){for(var h=[],g=0;g<e.insertAll.length;g++){for(var m=e.insertAll[g],o=Container.resolveType(m.type).memberDefinitions.getPublicMappedProperties(),k=0;k<o.length;k++){var i=o[k];if(i.concurrencyMode===$data.ConcurrencyMode.Fixed)m.data[i.name]=0;else if(i.computed)"string"===typeof m.data[i.name]&&
...
(m.data[i.name]=m.data[i.name].initData)}}h.push(m.data)}f.insert(h,{safe:!0},function(i,h){if(i)b.error(i);else{for(var g=0;g<h.length;g++)for(var m=h[g],k=e.insertAll[g],l=Container.resolveType(k.type).memberDefinitions.getPublicMappedProperties(),p=0;p<l.length;p++){var j=l[p];k.entity[j.name]=c._typeFactory(j.type,m[j.computed?"_id":j.name],c.fieldConverter.fromDb)}d+=h.length;e.removeAll&&e.removeAll.length?s(a,e,f):e.updateAll&&e.updateAll.length?n(a,e,f):r(a,d)}})},n=function(a,e,f){h=e.updateAll.length;
...
c.fieldConverter.toDb),q.$inc||(q.$inc={}),q.$inc[l.name]=1;else if(!l.computed&&"undefined"!==typeof j.entity[l.name])if(Container.resolveType(l.type)===$data.Array&&l.elementType&&Container.resolveType(l.elementType)===$data.ObjectID){q[l.name]=c._typeFactory(l.type,j.entity[l.name],c.fieldConverter.toDb);var n=q[l.name];if(n)for(i=0;i<n.length;i++)n[i]=c._typeFactory(l.elementType,n[i],c.fieldConverter.toDb)}else q[l.name]=c._typeFactory(l.type,j.entity[l.name],c.fieldConverter.toDb)}(function(e){f.update(m,
{$set:q},{safe:!0},function(i,k){if(i)b.error(i);else if(k){d++;for(var l=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),j=0;j<l.length;j++){var p=l[j];p.concurrencyMode===$data.ConcurrencyMode.Fixed&&e.entity[p.name]++}g(function(){r(a,d)})}else h--,f.find({_id:m._id},{}).toArray(function(i,f){if(i)b.error(i);else{for(var h=f[0],k=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),l=0;l<k.length;l++){var j=k[l];e.entity[j.name]=c._typeFactory(j.type,
h[j.computed?"_id":j.name],c.fieldConverter.fromDb)}g(function(){r(a,d)})}})})})(j)}},s=function(a,e,f){h=e.removeAll.length;for(var p=0;p<e.removeAll.length;p++){for(var j=e.removeAll[p],m=Container.resolveType(j.type).memberDefinitions.getKeyProperties(),o=0;o<m.length;o++){var k=m[o];j.data[k.computed?"_id":k.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(k.type))](j.entity[k.name])}m=Container.resolveType(j.type).memberDefinitions.getPublicMappedProperties();for(o=0;o<
m.length;o++)k=m[o],k.computed||(j.data[k.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(k.type))](j.data[k.name]),"undefined"===typeof j.data[k.name]&&delete j.data[k.name]),k.concurrencyMode!==$data.ConcurrencyMode.Fixed&&delete j.data[k.name];f.remove(j.data,{safe:!0},function(c,j){c?b.error(c):(j?d++:h--,g(function(){e.updateAll&&e.updateAll.length?n(a,e,f):r(a,d)}))})}},t=Object.keys(a),r=function(d,f){if(t.length){var h=t.pop();if(a.hasOwnProperty(h)){var g=a[h],h=new c.driver.Collection(d,
h);g.insertAll&&g.insertAll.length?e(d,g,h):g.removeAll&&g.removeAll.length?s(d,g,h):g.updateAll&&g.updateAll.length?n(d,g,h):(b.success(0),d.close())}}else b.success(f),d.close()};(new this.driver.Db(this.providerConfiguration.databaseName,f,{})).open(function(a,d){a?b.error(a):c.providerConfiguration.username?d.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(a,c){a?b.error(a):c&&r(d)}):r(d)})},saveChanges:function(b,a){if(a.length){for(var c=this.buildIndependentBlocks(a),
d=[],f={},h=0;h<c.length;h++)for(var g=0;g<c[h].length;g++){d.push(c[h][g].data);var e=f[c[h][g].entitySet.name];e||(e={},f[c[h][g].entitySet.name]=e);var n={entity:c[h][g].data,data:this.save_getInitData(c[h][g],d),type:Container.resolveName(c[h][g].data.getType())};switch(c[h][g].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:e.insertAll||(e.insertAll=[]);e.insertAll.push(n);break;case $data.EntityState.Modified:e.updateAll||(e.updateAll=[]);e.updateAll.push(n);
break;case $data.EntityState.Deleted:e.removeAll||(e.removeAll=[]);e.removeAll.push(n);break;default:Guard.raise(new Exception("Not supported Entity state"))}}this._saveCollections(b,f)}else b.success(0)},save_getInitData:function(b,a){b.physicalData=this.context._storageModel.getStorageModel(b.data.getType()).PhysicalType.convertTo(b.data,a);var c={};b.physicalData.getType().memberDefinitions.asArray().forEach(function(a){if(a.kind==$data.MemberTypes.navProperty||a.kind==$data.MemberTypes.complexProperty||
...
b?new $data.Array:b},"$data.ObjectID":function(b){return b?(new Buffer(b.toString(),"ascii")).toString("base64"):b},"$data.Geography":function(b){return void 0===b?new $data.Geography:new $data.Geography(b[0],b[1])}},toDb:{"$data.Integer":function(b){return b},"$data.Number":function(b){return b},"$data.Date":function(b){return b},"$data.String":function(b){return b},"$data.Boolean":function(b){if("string"===typeof b)switch(b){case "true":case "true":case "TRUE":case "yes":case "Yes":case "YES":return!0;
default:return!1}return null===b||void 0===b?null:!!b},"$data.Blob":function(b){return b},"$data.Object":function(b){return b},"$data.Array":function(b){return b},"$data.ObjectID":function(b){return b&&"string"===typeof b?new $data.mongoDBDriver.ObjectID.createFromHexString((new Buffer(b,"base64")).toString("ascii")):b},"$data.Geography":function(b){return b?[b.longitude,b.latitude]:b}}}}},{isSupported:{get:function(){return!$data.mongoDBDriver?!1:!0},set:function(){}}});
